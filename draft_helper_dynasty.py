...
# except OSError:
#    pass
conn = sql.connect('dynasty_ranks.db')
c = conn.cursor()
...
col_list.append('id')
player_cols_to_print = ['name', 'team', 'bye', 'rw_rank', 'diff']
...
def load_data(connection, cursor):
    ...
    # load in csv data
    with open("dynasty_ranks.csv", 'rb') as infile:
        r = csv.reader(infile)
        ...
        for row in r:
            ...
            if rotoworld_rank != 'NULL' and cbs_rank != 'NULL':
                diff = rotoworld_rank - cbs_rank
            elif cbs_rank != 'NULL':
                diff = -1 * cbs_rank
            ...
            t = (name, position, diff, cbs_rank, bye, rotoworld_rank, team, 0, None)
            print t
            cursor.execute(s, t)
            connection.commit()
...
def get_color_from_tier(position, t):
    return "white"
...
def get_top_by_position(position, num=20):
    l = get_players_by_position(c, position, orderby='rw_rank')
    if num < len(l):
        return l[0:num]
    else:
        return l
...
def augment_values_pick_diff(player_data, col_list, num_players_drafted=None, cursor=None):
    ...
    i = pl_col_list.index("cbs_rank")
    pl_col_list.insert(i, "cbs pick diff")
    pl_data_list = None
    if player_data:
        cbs_rank = get_data_from_player_row(player_data, 'cbs_rank')
        if str(cbs_rank) == 'NULL':
            cbs_pick_diff = 'NULL'
        else:
            cbs_pick_diff = num_players_drafted - cbs_rank
        pl_data_list = list(player_data)  # ensure list
        i = pl_data_list.index(cbs_rank)
        pl_data_list.insert(i, cbs_pick_diff)
    ...
    def populate_data_panes(self):
        ...
        for i, p in enumerate(all_top_players):
            ...
            self.all_players_pane.insert("", tk.END, values=zip(augmented_cols, augmented_vals),
                                         tags=(get_color_from_tier(pos, 0)))
